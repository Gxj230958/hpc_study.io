...
读完本章，你应能把数组分给多个工作者，正确合并结果，并解释为什么更多线程不一定更快。建议先读[执行模型](../thread-process/intro.md)，再运行 [OpenMP 求和](openmp.md)和 [mpi4py 分块](mpi4py.md)。后面的原有导论提供历史与设计方法，可作为补充阅读。
...
并行加速会受到串行部分和通信开销的限制。Amdahl 定律描述固定工作量下的一个理想上限，并不意味着所有程序已经达到这个上限。改进算法、数据布局与专用计算单元，都可能改善性能或能效。后面会介绍适合矩阵运算的 Tensor Core。

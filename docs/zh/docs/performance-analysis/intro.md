...
先运行[实验索引](../learning/labs.md)中的一个小程序，确认结果正确。然后读[Benchmark 测量方法](../benchmark/intro.md)，固定输入、线程数和计时范围。最后再使用下面的分析工具找热点。一次只改变一个因素，保留原始记录。

练习：某个函数占总时间的 10%，加速它两倍后，总时间理想上只减少 5%。因此应同时看局部速度和整体耗时。
...
这段代码只是读取计数器的示意，并不能直接保证精确计时。还需处理指令排序、计数器频率、线程迁移与测量开销；TSC 也不一定按当前核心实际频率计数。入门优先使用 `omp_get_wtime`、`MPI_Wtime` 或适当同步的 CUDA event。

...
初学者先选择一个工具回答一个问题：CPU 求和用墙钟时间观察线程数变化，GPU 应用先用 [Nsight Systems](../gpu/nsys.md)查看等待与传输。采样结果可能带来额外开销；工具不可用或权限不足时，应记录原因，不把空报告当作成功测量。练习与运行命令见[实验索引](../learning/labs.md)。
